...
function computeRef(job) {
  ...
  const { fn } = makeEscape(params);
  const coarse = coarsePass(fn, width, height, 8);

  // arrays we hand off (and lose) to the main thread for tile distribution
  const transfers = [coarse.sn.buffer];
